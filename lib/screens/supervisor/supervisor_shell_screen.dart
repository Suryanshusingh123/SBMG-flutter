...
import 'supervisor_complaints_screen.dart';
import 'supervisor_attendance_screen.dart';
import 'supervisor_settings_screen.dart';

/// Shell that wraps the main supervisor tab screens with a single bottom navigation bar.
/// Uses IndexedStack to keep tab state when switching. Detail screens (complaint
/// details, QR scanner, etc.) are pushed on top and do not show the bottom bar.
class SupervisorShellScreen extends StatefulWidget {
  /// Initial tab index (0=Home, 1=Complaints, 2=Attendance, 3=Settings).
  /// Used when navigating from login/redirect to a specific tab.
  final int initialIndex;

  const SupervisorShellScreen({super.key, this.initialIndex = 0});
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: IndexedStack(
          ...
          children: [
            const SupervisorHomeScreen(isEmbeddedInShell: true),
            const SupervisorComplaintsScreen(isEmbeddedInShell: true),
            const SupervisorAttendanceScreen(isEmbeddedInShell: true),
            const SupervisorSettingsScreen(isEmbeddedInShell: true),
          ],
        ),
